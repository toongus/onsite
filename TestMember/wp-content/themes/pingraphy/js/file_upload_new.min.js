    ...
    a.support.fileInput = !(new RegExp("(Android (1\\.[0156]|2\\.[01]))|(Windows Phone (OS 7|8\\.0))|(XBLWP)|(ZuneWP)|(WPDesktop)|(w(eb)?OSBrowser)|(webOS)|(Kindle/(1\\.0|2\\.[05]|3\\.0))").test(window.navigator.userAgent) || a('<input type="file">').prop("disabled")), a.support.xhrFileUpload = !(!window.ProgressEvent || !window.FileReader), a.support.xhrFormDataFileUpload = !!window.FormData, a.support.blobSlice = window.Blob && (Blob.prototype.slice || Blob.prototype.webkitSlice || Blob.prototype.mozSlice), a.widget("blueimp.fileupload", {
        ...
        _initXHRData: function(b) {
            ...
            b.headers = a.extend({}, b.headers), b.contentRange && (b.headers["Content-Range"] = b.contentRange), f && !b.blob && this._isInstanceOf("File", e) || (b.headers["Content-Disposition"] = 'attachment; filename="' + encodeURI(e.name) + '"'), f ? a.support.xhrFormDataFileUpload && (b.postMessage ? (c = this._getFormData(b), b.blob ? c.push({
                ...
                (d._isInstanceOf("File", e) || d._isInstanceOf("Blob", e)) && c.append(b.paramName[a] || g, e, e.uploadName || e.name)
            ...
            if (!n || l && void 0 !== k[0].size || (n = void 0), (j.singleFileUploads || m || n) && this._isXHRUpload(j))
                if (j.singleFileUploads || n || !m)
                    if (!j.singleFileUploads && n)
                        for (f = [], d = [], g = 0; l > g; g += 1) p += k[g].size + o, (g + 1 === l || p + k[g + 1].size + o > n) && (f.push(k.slice(r, g + 1)), e = q.slice(r, g + 1), e.length || (e = q), d.push(e), r = g + 1, p = 0);
                    else d = q;
            else
        ...
        processActions: {
            ...
            saveImage: function(b, c) {
                ...
                return b.canvas.toBlob ? (b.canvas.toBlob(function(a) {
                    a.name || (e.type === a.type ? a.name = e.name : e.name && (a.name = e.name.replace(/\..+$/, "." + a.type.substr(6)))), e.type !== a.type && delete b.imageHead, b.files[b.index] = a, f.resolveWith(d, [b])
...
function(a) {
    "use strict";
    "function" == typeof define && define.amd ? define(["jquery", "tmpl", "./jquery.fileupload-image", "./jquery.fileupload-audio", "./jquery.fileupload-video", "./jquery.fileupload-validate"], a) : a(window.jQuery, window.tmpl)
}(function(a, b, c) {
    "use strict";
    a.blueimp.fileupload.prototype._specialOptions.push("filesContainer", "uploadTemplateId", "downloadTemplateId"), a.widget("blueimp.fileupload", a.blueimp.fileupload, {
        options: {
            ...
            add: function(b, c) {
                ...
                var d = a(this),
                    ...
                    f = e.options;
                c.context = e._renderUpload(c.files).data("data", c).addClass("processing"), f.filesContainer[f.prependFiles ? "prepend" : "append"](c.context), e._forceReflow(c.context), a.when(e._transition(c.context), c.process(function() {
            ...
            fail: function(b, c) {
                ...
                c.context ? c.context.each(function(g) {
                    if ("abort" !== c.errorThrown) {
                        var h = c.files[g];
                        h.error = h.error || c.errorThrown || !0, e = f._addFinishedDeferreds(), f._transition(a(this)).done(function() {
                            var g = a(this);
            ...
            processstart: function(b) {
                return b.isDefaultPrevented() ? !1 : void a(this).addClass("fileupload-processing")
            },
            processstop: function(b) {
                return b.isDefaultPrevented() ? !1 : void a(this).removeClass("fileupload-processing")
            },
            destroy: function(b, c) {
                ...
                c.url ? (c.dataType = c.dataType || d.options.dataType, a.ajax(c).done(e)) : e()
            }
        },
        ...
        _cancelHandler: function(b) {
            ...
            var c = a(b.currentTarget).closest(".template-upload,.template-download"),
                d = c.data("data") || {};
            d.jqXHR ? d.jqXHR.abort() : (d.context = d.context || c, d.errorThrown = "abort", this._trigger("fail", b, d))
        },
...
function(a) {
    "use strict";
    "function" == typeof define && define.amd ? define(["jquery", "./jquery.fileupload-ui"], a) : a(window.jQuery)
}(function(a) {
    "use strict";
    a.widget("blueimp.fileupload", a.blueimp.fileupload, {
        options: {
            progress: function(a, b) {
                b.context && b.context.find(".progress").progressbar("option", "value", parseInt(b.loaded / b.total * 100, 10))
            },
            progressall: function(b, c) {
                var d = a(this);
                d.find(".fileupload-progress").find(".progress").progressbar("option", "value", parseInt(c.loaded / c.total * 100, 10)).end().find(".progress-extended").each(function() {
                    a(this).html((d.data("blueimp-fileupload") || d.data("fileupload"))._renderExtendedProgress(c))
                })
            }
        },
        _renderUpload: function(b, c) {
            var d = this._super(b, c),
                e = a(window).width() > 480;
            return d.find(".progress").empty().progressbar(), d.find(".start").button({
                icons: {
                    primary: "ui-icon-circle-arrow-e"
                },
                text: e
            }), d.find(".cancel").button({
                icons: {
                    primary: "ui-icon-cancel"
                },
                text: e
            }), d.hasClass("fade") && d.hide(), d
        },
        _renderDownload: function(b, c) {
            var d = this._super(b, c),
                e = a(window).width() > 480;
            return d.find(".delete").button({
                icons: {
        ...
        _transition: function(b) {
            var c = a.Deferred();
            return b.hasClass("fade") ? b.fadeToggle(this.options.transitionDuration, this.options.transitionEasing, function() {
                c.resolveWith(b)
            }) : c.resolveWith(b), c
        },
        _create: function() {
            this._super(), this.element.find(".fileupload-buttonbar").find(".fileinput-button").each(function() {
        ...
        _destroy: function() {
            ...
            }).end().find(".start").button("destroy").end().find(".cancel").button("destroy").end().find(".delete").button("destroy").end().find(".progress").progressbar("destroy"), this._super()
        }
    })
});